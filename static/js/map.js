(function ($) {
    ...
    mapFeatures.forEach(function (v) {
        ...
        // 地区经纬度
        ChinaGeoCoordMap[name] = v.properties.cp;

    });

    
    var worldGeoCoordMap_zh = {
    ...
    var countryName_zh_To_en_Map = {};

    for(let en_name in countryName_en_To_zh_Map){
        countryName_zh_To_en_Map[countryName_en_To_zh_Map[en_name]] = en_name;
    }
    
    var worldGeoCoordMap = {};

    for(let country_name_zh in worldGeoCoordMap_zh){
        worldGeoCoordMap[countryName_zh_To_en_Map[country_name_zh]] = worldGeoCoordMap_zh[country_name_zh];
    }
    ...
    function Map($el, options) {
        this.$el = $el;
        this.currentYear = Object.keys(options.data)[0];;
    ...
    Map.prototype.__bindEvent = function(){
        this.chart.on('click', (params) => {//点击事件
            // console.log(params)
            if (typeof(params.data) == 'object' && params.data.name) {
                let name = params.data.name;

                if(name in countryName_en_To_zh_Map){
                    this.options.onAreaClick && this.options.onAreaClick(countryName_en_To_zh_Map[name], this.currentYear);
                }else{
                    this.options.onAreaClick && this.options.onAreaClick(name, this.currentYear, this.options.mapType == 'china');
                }
            }else if(params.componentType == "timeline" && params.componentSubType == "slider"){
                this.currentYear = params.name;
                this.options.onYearChange && this.options.onYearChange(this.currentYear);
            }
        });
    }

    Map.prototype.update = function (options) {
        if(options.mapType != this.options.mapType){
            this.currentYear = Object.keys(options.data)[0];
        }
    ...
    Map.prototype.getEchartsOption = function () {
        let options = this.options;
        let data__many_year = options.data;     
        var mapName = this.options.mapType;

        var heatmapOption = {
            baseOption: {
                backgroundColor: '',
                timeline: {
                    data: Object.keys(data__many_year),
                    axisType: 'category',
                    lineStyle:{
                        color: '#fff'
                    },
                ...
                visualMap: {
                    ...
                    seriesIndex: [1],
                    inRange: {
                        color: ['rgba(159,205,253,0.0)','rgba(159,205,253,0.50)','#60ACFC']
                    },
                    textStyle: {
                        color: '#ccc'
                    }
            ...
            options: Object.keys(data__many_year).map((year) => {

                let data = this._buildNameValueData(data__many_year[year]);

                let {min_value, max_value} = this._getMinAndMaxValue(data);
                return {
                    visualMap: {
                        min: min_value,
                        max: max_value
                        ...
                        label: {
                            normal: {
                                formatter: function(params){
                                    if(mapName == 'world'){
                                        return countryName_en_To_zh_Map[params.name]
                                    }else{
                                        return params.name
                                    }
                                },
                    ...
                    },
                    {
                        ...
                        type: 'effectScatter',
                        coordinateSystem: 'geo',
                        data: this._buildCoorData(data.sort(function (a, b) {
                            return b.value - a.value;
                        }).slice(0, 5)),
                        symbolSize: function (val) {
                            return 10 + Math.log(val[2]);
                        },
                        showEffectOn: 'render',
                        rippleEffect: {
                            brushType: 'stroke'
    ...
    Map.prototype._getMinAndMaxValue = function(data){
        let min_value = Infinity;
        let max_value = -Infinity;

        data.forEach(item => {
            if(min_value > item.value){
    ...
    Map.prototype.override = function (chartOption) {
        if (this.options.override) {
            this.options.override(chartOption)
        }
    };

    Map.prototype._buildNameValueData = function (placeValueMap) {
        let name_value_data = [];
        for (let name in placeValueMap) {
            let value = placeValueMap[name];
            if(this.options.mapType == 'world'){
                name = countryName_zh_To_en_Map[name];
            }
            name_value_data.push({
                name: name,
                value: value
            });
        }

        return name_value_data;
    }

    Map.prototype._buildCoorData = function (placeValueMap_Array) {
        let geoCoordMap = this.options.mapType == 'china' ? ChinaGeoCoordMap : worldGeoCoordMap


        var res = [];
        for (var i = 0; i < placeValueMap_Array.length; i++) {
            let placeValueMap = placeValueMap_Array[i];

            let placeName = placeValueMap.name;

            var geoCoord = geoCoordMap[placeName];
            if (geoCoord) {
                res.push({
                    name: placeName,
                    value: geoCoord.concat(placeValueMap.value),
                });
            }else{
